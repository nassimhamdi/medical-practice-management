...
public interface UserRepository extends JpaRepository<UserEntity,Integer> {
    ...
    @Query(value = "insert into users values(:id,:firstName,:lastName,:email"
            + ",:password,:cellNo,:role)",nativeQuery = true)
    int insertIntoUsers(@Param("id") int userId,@Param("firstName") String firstName,
                        @Param("lastName") String lastName,@Param("email") String email,@Param("password") String password,@Param("cellNo") String cellNo,@Param("role") String role);
